...
class m151103_090004_create_lc_building_translation extends Migration
{
    public function up()
    {
    	$this->execute("CREATE TABLE `lc_building_translation`(  
			`id` INT,
			...
			CONSTRAINT `lc_building_translation:id&lc_building:id` FOREIGN KEY (`id`) REFERENCES `lc_building`(`id`)
		);");

    	$this->execute("DELETE FROM `language_translate`;");
    	$this->execute("DELETE FROM `language_source`;");

    	$this->execute("UPDATE `language` SET `status` = 0;");
    	$this->execute("UPDATE `language` SET `status` = 1 WHERE `language_id` = 'en-US' OR `language_id` = 'vi-VN';");
    ...
    {
        $this->dropTable('`lc_building_translation`');
    }
    
    public function getActiveLangs() {
    	$connection = \Yii::$app->db;
    	$model = $connection->createCommand('SELECT * FROM `language` WHERE `status` = 1');

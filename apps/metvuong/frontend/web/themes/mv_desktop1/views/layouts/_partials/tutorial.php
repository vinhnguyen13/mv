...
if($action == 'site/index' && (!isset($cookie['Homepage']) || empty($cookie['Homepage']))){
    ...
    $txtArr = [
        ...
        Yii::t("tutorial","Cập nhật trạng thái các tin đăng của bạn gồm có:<p class='mgB-5'>Lượt tìm kiếm tin đăng,</p><p class='mgB-5'>Lượt yêu thích,</p><p class='mgB-5'>Lượt chia sẻ.</p>"),
        ...
        Yii::t("tutorial",'Trang Trò Chuyện hiển thị "conversation history" của những người mà bạn đã chat với.'),
    ];
    ?>
    <script>
        $(document).ready(function () {
    ...
    <script>
        $(document).ready(function () {
            textTour(<?=json_encode($txtArr)?>, 'Payment');
        });
    </script>
    <?php
    $this->registerJsFile(Yii::$app->view->theme->baseUrl.'/resources/js/tour-intro.js', ['position'=>View::POS_END]);

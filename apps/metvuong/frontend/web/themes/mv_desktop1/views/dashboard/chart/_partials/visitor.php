    ...
    <script>
        $(function () {
            $('#chartAds').highcharts({
                ...
                title: {
                    text: '<?=Yii::t('chart', 'The visitor user')?>',
                    x: -20 //center
                },
                subtitle: {
...
<div class="statistic-info">
    ...
    <?php if($from > 0 && $to > 0) {?>
        <p class="date-filter-chart text-center mgT-15 fs-14"><?=Yii::t('chart', 'Statistic visit from')?> <span class="from"><?=date('d/m/Y', $from)?></span> - <span class="to"><?=date('d/m/Y', $to)?></span></p>
    <?php } ?>
</div>

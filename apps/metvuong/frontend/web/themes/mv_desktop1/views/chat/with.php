...
<script class="chat-box-template" type="text/x-handlebars-template">
	<div class="container">
		<div class="chat-real chat-group" chat-from="{{from}}" chat-to="{{to}}">
			<div class="title-top"><a href="/<?=Yii::$app->language?>/{{toAlias}}" title="<?=Yii::t('chat', 'View profile')?>" target="_blank">{{toName}}</a></div>
...
<script>
	$(document).ready(function () {
		...
		chatUI.showBoxChat(xmpp_jid, '<?=$to;?>', {fromName: '<?=$nameUserFrom;?>', toName: '<?=$nameUserTo;?>', toAlias: '<?=$userTo->getUsername()?>'});

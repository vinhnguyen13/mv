...
!function($, window, document, _undefined)
{
	...
	$.extend(XenForo,
	{
		...
		init: function()
		{
			...
			//disable deferred.php auto crontab
/*
...
*/			
		...
		updateVisibleNavigationTabs: function()
		{
			...
			$publicInnerTabs.filter('.navigationHiddenTabs').hide();
			...
			var hidePublicTabs = function()
				{
					...
					var $hidden = $publicInnerTabs.filter(':not(' + shownSel + ')');
					$hidden.hide();
					...
					$publicInnerTabs.filter(shownSel).show();
				},
			...
			$element.find('.bbCodeQuote .quoteContainer').each(function() {
				var self = this,
					...
					checkHeight = function() {
						...
						if (quote.scrollHeight > quote.offsetHeight)
						{
	 ...
	 */
        XenForo.DuyetLink = function($link)
	{
		$link.click(function(e)
		{
			e.preventDefault();
			...
			XenForo.ajax(this.href, {}, function(ajaxData, textStatus)
			{
				...
				if (ajaxData.kq) // term = Like / Unlike
				{
                                        var $span=$link.parent();
                                        var $username="<a href='/members/"+ajaxData.username+"."+ajaxData.userid+"'>"+ajaxData.username+"</a>";
					$span.html("<span class='daduyet'>Duyệt bởi: "+$username+"</span>");
				}
			});
		});
	};
		...
		$link.click(function(e)
		{
			...
			XenForo.ajax(this.href, {}, function(ajaxData, textStatus)
			{
				...
				if (ajaxData.term) // term = Like / Unlike
				{
					...
					{
					//add count like
						var count = $link.find('.LikeLabel').prev();
						if(count.text()){
							if(ajaxData.cssClasses.like == '+'){
									count.text(parseInt(count.text())-1)
							}else if(ajaxData.cssClasses.like == '-'){
									count.text(parseInt(count.text())+1)
							}
						}
					//add count like	
...
//		XenForo.register('.PreviewTooltip', 'XenForo.PreviewTooltip');
	...
	if ($('html').hasClass('Public'))
	{
		...
		XenForo.register('a.LikeLink', 'XenForo.LikeLink');
                ...
                XenForo.register('a.DuyetLink', 'XenForo.DuyetLink');

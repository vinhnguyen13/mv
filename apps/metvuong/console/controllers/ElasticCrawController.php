...
class ElasticCrawController extends Controller {
	...
	public function buildDocument($item, $district, $city, $isWard = false) {
		...
		return [
			...
			'total_sell' => 0,
			'total_rent' => 0,
			'city_id' => intval($district['city_id']),
			'district_id' => intval($district['district_id']),
			's1' => $name,
			's2' => $name,
			's3' => $nameWithPrefix,
			's4' => $acronym,
	...
	public function getAreas($table, $addSelect = []) {
		$select = ["`$table`.`id`", "`$table`.`name`", "`$table`.`pre`", "'' AS `slug`"];
		
		if($table == $this->tProjectTable) {
			$select = ["`$table`.`id`", "`$table`.`name`", "'Dự án' AS `pre`", "'' AS `slug`"];
		} else {
			$select = ["`$table`.`id`", "`$table`.`name`", "`$table`.`pre`", "'' AS `slug`"];
		}

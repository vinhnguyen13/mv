...
class ElasticController extends Controller {
		...
		$settings = [
			'analysis' => [
				'char_filter' => [
					'my_char_filter' => [
						'type' => 'mapping',
						'mappings' => $this->mapping(["/=>slash","hcm=>ho chi minh","cmt8=>cach mang thang tam","xvnt=>xo viet nghe tinh","ntmk=>nguyen thi minh khai", "hn=>ha noi"])
					]	
				],
				'filter' => [
					'my_synonym_filter' => [
	...
	private function buildTerm($id, $cityId, $name, $fullName, $totalSell, $totalRent, $slug) {
		...
		$searchName = $this->standardSearch(Elastic::transform($name));
		
		$term[] = [
			'name'	=> $name,
			'slug' => $slug,
			'search_name' => $searchName,
			'search_name_n' => $searchName,
			'full_name' => $fullName,

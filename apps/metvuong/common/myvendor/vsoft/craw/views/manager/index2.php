...
$columns = [
    ...
    'apca' => [
    	...
    	'label' => 'Ngày đăng',
		'filter' =>	Html::activeDropDownList($searchModel, 'created_filter', ['1' => 'Đang hoạt động', '2' => 'Đã hết hạn', '3' => 'Lọc theo range'], ['class' => 'form-control filter-type-2', 'prompt' => 'Tất cả', 'style' => $searchModel->created_filter == 3 ? 'display: none;' : '']) .
					'<div class="mask-wrap range-wrap"' . ($searchModel->created_filter != 3 ? ' style="display: none"' : '') . '>' .
					Html::activeTextInput($searchModel, 'created_mask', ['class' => 'form-control search-range search', 'readonly' => true]) .
					'<span class="mask-close">x</span><div class="range range-date-wrap">' .
					Html::activeTextInput($searchModel, 'date_from', ['class' => 'form-control range-value range-min range-date', 'placeholder' => 'From']) . ' - ' .
					Html::activeTextInput($searchModel, 'date_to', ['class' => 'form-control range-value range-max range-date', 'placeholder' => 'To']) .
					'<input type="button" class="submit-range btn btn-success" value="OK" style="margin-left: 8px;" />' .
					'</div></div>',
    	'value' => function($model) {

<?php

use yii\helpers\Html;
use vsoft\news\models\CmsCatalog;
use vsoft\news\Module;

/* @var $this yii\web\View */
/* @var $searchModel app\models\CmsCatalogSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = Module::t('cms', 'Cms Catalogs');
$this->params['breadcrumbs'][] = $this->title;
?>
<div class="cms-catalog-index">

    <?php // echo $this->render('_search', ['model' => $searchModel]); ?>

    <p>
        <?= Html::a(Module::t('cms', 'Create ') . Module::t('cms', 'Cms Catalog'), ['create'], ['class' => 'btn btn-success']) ?>
    </p>

    <table class="table table-striped table-bordered">
        <thead>
        <tr>
            <th>#</th>
            <th><?= Module::t('cms', 'Title') ?> </th>
            <th><?= Module::t('cms', 'Update At') ?></th>
            <th><?= Module::t('cms', 'Actions') ?></th>

        </tr>
        </thead>
        <tbody>
        <?php foreach($dataProvider as $k => $item){ ?>
        <tr data-key="1">
            <td><?= $k+1 ?></td>
            <td><?= Html::a($item['title'], ['view', 'id' => $item['id']], ['style' => ['text-decoration' => 'none']]); ?></td>
            <td><?= date("d-M-Y H:i:s", $item['updated_at']); ?></td>
            <?php // CmsCatalog::getCatalogPageTypeLabels($item['page_type']); ?>
            <?php // \vsoft\news\models\YesNo::labels()[$item['is_nav']]; ?>
            <?php // \vsoft\news\models\Status::labels()[$item['status']]; ?>
            <td>
                <?php if ($item['page_type'] == CmsCatalog::PAGE_TYPE_LIST) { ?><a href="<?= \Yii::$app->getUrlManager()->createUrl(['news/cms/create','catalog_id'=>$item['id']]); ?>" title="<?= Module::t('cms', 'Add Show');?>" data-pjax="0"><span class="glyphicon glyphicon-file"></span></a> <?php } ?>
                <a href="<?= \Yii::$app->getUrlManager()->createUrl(['news/cms-catalog/create','parent_id'=>$item['id']]); ?>" title="<?= Module::t('cms', 'Add Sub Catelog');?>" data-pjax="0"><span class="glyphicon glyphicon-plus-sign"></span></a>
                <a href="<?= \Yii::$app->getUrlManager()->createUrl(['news/cms-catalog/view','id'=>$item['id']]); ?>"" title="<?= Module::t('cms', 'View');?>" data-pjax="0"><span class="glyphicon glyphicon-eye-open"></span></a>
                <a href="<?= \Yii::$app->getUrlManager()->createUrl(['news/cms-catalog/update','id'=>$item['id']]); ?>"" title="<?= Module::t('cms', 'Update');?>" data-pjax="0"><span class="glyphicon glyphicon-pencil"></span></a>
                <a href="<?= \Yii::$app->getUrlManager()->createUrl(['news/cms-catalog/delete','id'=>$item['id']]); ?>"" title="<?= Module::t('cms', 'Delete');?>" data-confirm="<?= Module::t('cms', 'Are you sure you want to delete this item?');?>" data-method="post" data-pjax="0"><span class="glyphicon glyphicon-trash"></span></a>
            </td>
        </tr>
        <?php } ?>
        </tbody>
    </table>

</div>
